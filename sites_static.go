package main

// Code generated from resource_sites.json. Do not edit manually.

// defaultSitesJSON 内置资源站配置（与 resource_sites.json 一致，默认全部禁用）
const defaultSitesJSON = `{
    "version": "1.0",
    "update_date": "2026-09-12",
    "sites": [
        {
            "name": "量子",
            "site_url": "http://23.224.101.30",
            "api_url": "https://cj.lziapi.com/api.php/provide/vod/from/lzm3u8/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "推荐",
            "priority": 100
        },
        {
            "name": "暴风",
            "site_url": "http://bfzy8.tv",
            "api_url": "http://by.bfzyapi.com/api.php/provide/vod/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "推荐",
            "priority": 100
        },
        {
            "name": "非凡",
            "site_url": "https://www.ffzy.tv",
            "api_url": "https://www.ffzy.tv/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "推荐，API已更新",
            "priority": 100
        },
        {
            "name": "猫眼",
            "site_url": "https://www.maoyanzy.com",
            "api_url": "https://api.maoyanapi.top/api.php/provide/vod/from/mym3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "压缩其他资源站，插播都没去",
            "priority": 100
        },
        {
            "name": "最大",
            "site_url": "https://zuidazy.me",
            "api_url": "https://api.zuidapi.com/api.php/provide/vod/from/zuidam3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "API已更新",
            "priority": 100
        },
        {
            "name": "OK资源",
            "site_url": "https://okzyw.cc",
            "api_url": "https://api.okzyw.net/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "API已更新",
            "priority": 100
        },
        {
            "name": "快车",
            "site_url": "https://kuaichezy.vip",
            "api_url": "https://caiji.kuaichezy.org/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "API已更新",
            "priority": 100
        },
        {
            "name": "闪电",
            "site_url": "https://shandianzy.com",
            "api_url": "https://xsd.sdzyapi.com/api.php/provide/vod/from/sdm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "API已更新",
            "priority": 100
        },
        {
            "name": "丫丫（鸭鸭）",
            "site_url": "https://yayazy.com",
            "api_url": "https://cj.yayazy.net/api.php/provide/vod/from/yym3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "速播",
            "site_url": "https://www.subozy.com",
            "api_url": "https://subocj.com/api.php/provide/vod/from/subm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "新浪和速播链接相同",
            "priority": 100
        },
        {
            "name": "红牛",
            "site_url": "https://hongniuzy.net",
            "api_url": "https://www.hongniuzy2.com/api.php/provide/vod/from/hnm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "豪华",
            "site_url": "https://hhzyapi.com",
            "api_url": "https://hhzyapi.com/api.php/provide/vod/from/hhm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "光速",
            "site_url": "https://guangsuzy.net",
            "api_url": "https://api.guangsuapi.com/api.php/provide/vod/from/gsm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "13大众",
            "site_url": "https://www.dzzy.com",
            "api_url": "http://cdn.dzzyapi.com/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "仅支持HTTP",
            "priority": 100
        },
        {
            "name": "如意",
            "site_url": "https://www.ryzy.tv",
            "api_url": "https://www.ryzy.tv/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "API已更新",
            "priority": 100
        },
        {
            "name": "天繁",
            "site_url": "https://www.tianfanzy.com",
            "api_url": "https://caiji.dyttzyapi.com/api.php/provide/vod/from/dyttm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "IP验证无法播放，开全局代理或返回原始链接",
            "priority": 100
        },
        {
            "name": "西瓜",
            "site_url": "https://xgzy.tv",
            "api_url": "https://caiji.xgzyapi.com/api.php/provide/vod/at/xml/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "XML采集接口",
            "priority": 100
        },
        {
            "name": "蓝光",
            "site_url": "https://lgzyz.xyz",
            "api_url": "https://lgzyz.xyz/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "新增，4K资源",
            "priority": 100
        },
        {
            "name": "看看",
            "site_url": "https://kankan01.com",
            "api_url": "https://kankan01.com/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "新增",
            "priority": 100
        },
        {
            "name": "樱花",
            "site_url": "https://yhzy.men/",
            "api_url": "https://m3u8.apiyhzy.com/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "新增",
            "priority": 100
        },
        {
            "name": "好花",
            "site_url": "https://www.haohuazy.com",
            "api_url": "https://www.haohuazy.com/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "新增",
            "priority": 100
        },
        {
            "name": "百度",
            "site_url": "https://api.apibdzy.com/",
            "api_url": "https://api.apibdzy.com/api.php/provide/vod/from/dbm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "地区限制，部分无法打开",
            "priority": 100
        },
        {
            "name": "电影天堂",
            "site_url": "https://dyttzyw.tv",
            "api_url": "https://dyttzyw.tv/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "新增",
            "priority": 100
        },
        {
            "name": "爱奇艺资",
            "site_url": "https://iqiyizy.cc",
            "api_url": "https://iqiyizy.cc/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "新增",
            "priority": 100
        },
        {
            "name": "天逸",
            "site_url": "https://tyyszy.com",
            "api_url": "https://tyyszy.com/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "新增",
            "priority": 100
        },
        {
            "name": "1080zyku",
            "site_url": "https://api.1080zyku.com",
            "api_url": "https://api.1080zyku.com/inc/api_mac10.php/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "官方推荐",
            "priority": 100
        },
        {
            "name": "360资源",
            "site_url": "https://360zy.com",
            "api_url": "https://360zy.com/api.php/provide/vod/from/360zy/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "JSON接口",
            "priority": 100
        },
        {
            "name": "虎牙资源",
            "site_url": "https://www.huyaapi.com",
            "api_url": "https://www.huyaapi.com/api.php/provide/vod/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "JSON接口",
            "priority": 100
        },
        {
            "name": "百度资源",
            "site_url": "https://api.apibdzy.com",
            "api_url": "https://api.apibdzy.com/api.php/provide/vod/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "JSON接口",
            "priority": 100
        },
        {
            "name": "飘零资源",
            "site_url": "https://p2100.net",
            "api_url": "https://p2100.net/api.php/provide/vod/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "JSON接口",
            "priority": 100
        },
        {
            "name": "速博资源",
            "site_url": "https://subocaiji.com",
            "api_url": "https://subocaiji.com/api.php/provide/vod/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "JSON接口",
            "priority": 100
        },
        {
            "name": "魔都资源",
            "site_url": "https://caiji.moduapi.cc",
            "api_url": "https://caiji.moduapi.cc/api.php/provide/vod/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "JSON接口",
            "priority": 100
        },
        {
            "name": "森林资源",
            "site_url": "http://slapibf.com",
            "api_url": "http://slapibf.com/api.php/provide/vod/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "补充资源站",
            "priority": 100
        },
        {
            "name": "CK资源网",
            "site_url": "https://ckzy1.com",
            "api_url": "https://ckzy1.com/api.php/provide/vod/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "萌芽合作站",
            "priority": 100
        },
        {
            "name": "杏吧资源站",
            "site_url": "https://sex8zy8.com",
            "api_url": "https://sex8zy8.com/api.php/provide/vod/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "萌芽合作站",
            "priority": 100
        },
        {
            "name": "森林资源站",
            "site_url": "http://senlinzy3.com",
            "api_url": "http://senlinzy3.com/api.php/provide/vod/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "萌芽合作站",
            "priority": 100
        },
        {
            "name": "无水印资源",
            "site_url": "https://api.wsyzy.net",
            "api_url": "https://api.wsyzy.net/api.php/provide/vod/from/wsym3u8/at/json/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "闪电资源",
            "site_url": "https://xsd.sdzyapi.com",
            "api_url": "https://xsd.sdzyapi.com/api.php/provide/vod/from/sdm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "快车资源",
            "site_url": "https://caiji.kuaichezy.org",
            "api_url": "https://caiji.kuaichezy.org/api.php/provide/vod/from/kcm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "牛牛资源",
            "site_url": "https://api.niuniuzy.me",
            "api_url": "https://api.niuniuzy.me/api.php/provide/vod/from/nnm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "索尼资源",
            "site_url": "https://suoniapi.com",
            "api_url": "https://suoniapi.com/api.php/provide/vod/from/snm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "最大资源",
            "site_url": "https://api.zuidapi.com",
            "api_url": "https://api.zuidapi.com/api.php/provide/vod/from/zuidam3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "U酷资源API",
            "site_url": "https://api.ukuapi88.com",
            "api_url": "https://api.ukuapi88.com/api.php/provide/vod/from/ukm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "新浪资源",
            "site_url": "https://api.xinlangapi.com",
            "api_url": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/from/xlm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "爱奇艺站",
            "site_url": "https://iqiyizyapi.com",
            "api_url": "https://iqiyizyapi.com/api.php/provide/vod/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "金鹰资源API",
            "site_url": "https://jyzyapi.com",
            "api_url": "https://jyzyapi.com/provide/vod/from/jinyingm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "红牛资源API",
            "site_url": "https://www.hongniuzy2.com",
            "api_url": "https://www.hongniuzy2.com/api.php/provide/vod/from/hnm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "非凡资源API",
            "site_url": "https://cj.ffzyapi.com",
            "api_url": "http://cj.ffzyapi.com/api.php/provide/vod/from/ffm3u8/at/json/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "CK伦理",
            "site_url": "https://ckzy.me",
            "api_url": "https://ckzy.me/api.php/provide/vod/from/ckm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "天涯影视",
            "site_url": "https://tyyszyapi.com",
            "api_url": "https://tyyszyapi.com/api.php/provide/vod/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "极速资源API",
            "site_url": "https://jszyapi.com",
            "api_url": "https://jszyapi.com/api.php/provide/vod/from/jsm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "猫眼资源API",
            "site_url": "https://api.maoyanapi.top",
            "api_url": "https://api.maoyanapi.top/api.php/provide/vod/at/json/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "百度云站",
            "site_url": "https://bdzy1.com",
            "api_url": "https://bdzy1.com/api.php/provide/vod/from/dbm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "搜AV资源",
            "site_url": "https://api.souavzyw.net",
            "api_url": "https://api.souavzyw.net/api.php/provide/vod/at/json/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "桃花视频",
            "site_url": "https://thzy1.me",
            "api_url": "https://thzy1.me/api.php/provide/vod/from/thm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "黄色仓库",
            "site_url": "https://hsckzy888.com",
            "api_url": "https://hsckzy888.com/api.php/provide/vod/from/hsckm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "杏吧资源",
            "site_url": "https://xingba222.com",
            "api_url": "https://xingba222.com/api.php/provide/vod/at/json/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "玉兔资源API",
            "site_url": "https://apiyutu.com",
            "api_url": "https://apiyutu.com/api.php/provide/vod/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "CK百货",
            "site_url": "https://ckbh1.xyz",
            "api_url": "https://ckbh1.xyz/api.php/provide/vod/from/ckm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "建安资源",
            "site_url": "https://154.219.117.232:9981",
            "api_url": "http://154.219.117.232:9981/jacloudapi.php/provide/vod/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "精品98资源",
            "site_url": "https://jp98.vip",
            "api_url": "https://jp98.vip/api.php/provide/vod/at/json/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "暴风资源站S",
            "site_url": "https://bf.xoxowin86cisyap.com",
            "api_url": "https://bf.xoxowin86cisyap.com/api.php/provide/vod/at/json/",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "检测可用",
            "priority": 100
        },
        {
            "name": "优质",
            "site_url": "https://yzzy.tv",
            "api_url": "https://api.yzzy-api.com/inc/api_mac10.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "推荐",
            "priority": 100
        },
        {
            "name": "无印",
            "site_url": "https://wsyzy.cc",
            "api_url": "https://wsyzy.cc/api.php/provide/vod/from/wym3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "卡",
            "priority": 60
        },
        {
            "name": "魔都",
            "site_url": "https://moduzy2.com",
            "api_url": "https://www.mdzyapi.com/api.php/provide/vod/from/modum3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "U酷",
            "site_url": "http://www.ukuzy.com",
            "api_url": "https://api.ukuapi.com/api.php/provide/vod/from/ukm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "豆瓣",
            "site_url": "https://dbzy.tv",
            "api_url": "https://caiji.dbzy.tv/api.php/provide/vod/from/dbm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "天涯",
            "site_url": "https://tyyszy.com",
            "api_url": "https://tyyszyapi.com/api.php/provide/vod/from/tym3u8?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "爱奇艺",
            "site_url": "https://iqyapi.w1612.cc",
            "api_url": "https://iqyapi.w1612.cc/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "索尼",
            "site_url": "https://suonizy.com",
            "api_url": "https://suoniapi.com/api.php/provide/vod/from/snm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "牛牛",
            "site_url": "https://www.niuniuzy.org",
            "api_url": "https://api.niuniuzy.me/api.php/provide/vod/from/nnm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "无尽",
            "site_url": "https://www.wujinzy.net",
            "api_url": "https://api.wujinapi.me/api.php/provide/vod/from/wjm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "无尽（无水印）",
            "site_url": "https://wujinwszy.com",
            "api_url": "http://api.wujinwszy.net/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "无水印，但卡",
            "priority": 60
        },
        {
            "name": "新浪",
            "site_url": "https://www.xinlangzy.net",
            "api_url": "https://api.xinlangapi.com/xinlangapi.php/provide/vod/from/xlm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "新浪和速播链接相同",
            "priority": 60
        },
        {
            "name": "极速",
            "site_url": "http://www.jisuzy.com",
            "api_url": "https://jszyapi.com/api.php/provide/vod/from/jsm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "淘片",
            "site_url": "https://www.taopianzy.com/index.html",
            "api_url": "https://taopianapi.com/cjapi/mc10/vod/json/m3u8.html",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "茅台",
            "site_url": "https://mtzy.me",
            "api_url": "https://caiji.maotai999.vip/api.php/provide/vod/from/mtm3u8/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "虎牙",
            "site_url": "https://www.huyaapi.com",
            "api_url": "https://www.huyaapi.com/api.php/provide/vod/from/hym3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "金鹰",
            "site_url": "https://jinyingzy.com",
            "api_url": "https://jyzyapi.com/provide/vod/from/jinyingm3u8/at/json",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "大众",
            "site_url": "https://www.dzzy.com",
            "api_url": "https://cdn.dzzyapi.com/api.php/provide/vod/?ac=list",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        },
        {
            "name": "ikun",
            "site_url": "https://ikunzyapi.com",
            "api_url": "https://ikunzyapi.com/api.php/provide/vod",
            "type": "maccms",
            "status": "active",
            "enabled": true,
            "note": "",
            "priority": 60
        }
    ]
}`
