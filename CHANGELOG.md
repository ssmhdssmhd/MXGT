...
## Go 分支 v0.6.35 (2026-09-13) — AI 视觉识别压字广告位置，自动遮挡任意位置

> 用户诉求：压字广告/水印/滚动字幕"各种位置都有"，需要自动识别并自动遮挡。

### 1. 方案

固定顶部/底部遮罩条只能盖固定位置。改为**视觉大模型识别**：截取当前视频帧(JPEG base64)发给支持"看图"的多模态模型，模型返回画面内所有压字广告/水印/滚动字幕所在的归一化矩形(0~1)，播放器据此在这些位置叠加遮罩块——位置不再固定，顶/底/四角/中段都能盖。支持 OpenAI 兼容 / Anthropic / Gemini 三大多模态协议。

### 2. 改动（[main.go](file:///workspace/main.go)）

- **配置扩展**：`OverlayMaskConfig` 新增 `rects []OverlayRect{x,y,w,h,label}`（归一化任意矩形遮罩）；后端 GET 返回、POST 保存时一并带出；
- **AI 视觉识别**：新增 `aiVisionDetect(mime,data,cfg)`（按 openai/anthropic/gemini 组装多模态消息，接受 image_url/inline_data/image source，解析 JSON 数组坐标并做合法性过滤）；
- **识别接口**：新增 `POST /api/overlay/detect`（需登录），body `{image:jpeg base64,mime, save}`；识别结果可选写回全局 `rects` 并自动 `enabled=true`（全局生效）；
- **前端-外置播放页 `/player`**：video 下方新增「🎯 自动识别压字广告位置」按钮——截当前帧 → 调 detect → 保留全局配置仅替换 rects 并叠加任意位置遮罩；`applyOverlay` 支持渲染 `rects`；
- **前端-后台**：「🧱 压字广告遮挡」面板新增「🎯 AI 识别压字位置」按钮 + 已识别到的位置清单展示（`showOvlRects`）；内置播放器同样叠加 rects；
- 路由注册 `/api/overlay/detect`。

### 3. 使用说明

1. 保证「AI 大模型接入」的**🖼️ 视觉识别模型**填入支持看图的多模态模型（预填免费的智谱 `glm-4v-flash`，也支持 `glm-4v-plus` / gpt-4o / Qwen-VL）；去广告/官替仍用下方免费文本模型 `glm-4-flash` 不受影响；
2. 在 `/player?url=<源>` 播放，或后台内置播放器播放；
3. 点「🎯 自动识别压字广告位置」→ 画面中出现该片压字广告的一帧（让压字可见）→ 识别保存 → 自动全局叠加任意位置遮罩。

### 4. 验证与版本

- `go build` + `go vet` 通过；版本升级 `v0.6.34 → v0.6.35`。

---

## Go 分支 v0.6.34 (2026-09-12) — 压字广告遮挡：画面内水印/压字/滚动字幕前端遮挡

> 用户诉求：部分广告是压缩在视频里面（画面内烧录，如「本片由 69794.com 赞助发布：主营 棋牌/体育/真人娱乐/电子/捕鱼」），有没有办法去除或遮挡。

### 1. 说明

烧录进视频像素的压字广告没有独立片段可删，m3u8 去广告/非正片区间跳过均无法去除，只能**前端遮挡**：半透明遮罩条盖住顶部/底部固定水印与滚动字幕条，或把画面放大裁边把边缘压字挤出可视区。两者都在浏览器端实时完成，不转码、不引入延迟、不影响秒拖秒播。

### 2. 改动（[main.go](file:///workspace/main.go)）

- **全局遮挡配置**：新增 `overlay_masks.json`（`OverlayMaskConfig`：总开关/顶部条高度%/底部条高度%/遮罩颜色透明度/放大倍数），`GET /api/overlay` 开放读取（播放器用）、`POST /api/overlay/save` 需登录保存；
- **播放器叠加**：内置播放器（后台）+ 外置播放页（`/player`）在视频上叠 `position:absolute` 遮罩条（高度按画面 %）+ `transform:scale` 放大裁边；
- **URL 参数临时覆盖**：`/player?url=…&overlay=on|off` 强制启用/关闭全局；`overlay=top:10,bottom:8,zoom:1.1,opacity:0.5,color:000` 精确指定（逗号/分号分隔），满足单个链接不同遮挡需求；
- **后台设置区**：adminPageHTML 新增「🧱 压字广告遮挡」面板（开关/顶部/底部/高度/放大倍数/透明度/颜色），保存后播放器即时生效；
- 接口说明表补充 `/api/overlay` 条目。

### 3. 验证与版本

- `go build` + `go vet` 通过；版本升级 `v0.6.33 → v0.6.34`。
...
### 1. 实测结果（智谱 glm-4-flash + 用户真实链接）

- **场景 1 腾讯官替 ✅**：`https://m.v.qq.com/x/m/play?cid=mzc00200jtsx0ds&vid=q4102f69upp` → AI 识别出「交锋 / 第1集」→ 全资源站匹配 → 生成无广告直链；
- **场景 2 m3u8 去广告 ✅**：`https://fengbao12.com/video/jiaofeng/805819b66508/index.m3u8`（951 段）→ 修复后识别并删除 **18 段真实插播广告**（`/video/adjump/` 路径，100-108 与 809-817 两处）+ 超短片段共 32 段，保留 919 段。

### 2. 修复内容（[main.go](file:///workspace/main.go)）

- **adjump 广告漏检**：该 m3u8 广告路径 `/video/adjump/time/xxx.ts` 不在原 URL 关键词正则内，规则引擎完全漏掉。`adURLRe` 补齐 `adjump|jumpad|adtime|adinsert`（路径分隔符边界整词匹配，保守不误伤）；
- **AI 幻觉保护**（免费小模型实测发现）：智谱对 300 段列表把大片正片误标为广告（194-299 全标）。新增两道防线：
  - `filterAIRuns`：只保留长度 ≤20 的连续标记簇；
  - `aiAdRatioGuard`：AI 判定占比 >20% 视为幻觉，整体丢弃并给出原因（宁可不删也不能误删整段正片）；
- **官替链路 panic 修复**：`episodeNumOfPlayItem` 对不匹配集数正则的播放项名称（如纯剧名「庆余年」）`m[1:]` 切片越界直接 panic（实测复现），加 `len(m)>1` 保护。

### 3. 验证与版本

- 本地端到端实测两个场景均通过；`go build` + `go vet` 通过；版本升级 `v0.6.28 → v0.6.29`。

---

## Go 分支 v0.6.28 (2026-09-12) — AI 参与/失败状态可见：免费模型接入可排查

> 用户诉求：用免费 AI 测试「AI 智能官替（腾讯视频识别剧名/集数）」与「AI 去广告（m3u8 识别去除插播/广告）」时，无法判断 AI 到底有没有参与、失败原因是什么。

### 1. 根因（[main.go](file:///workspace/main.go)）

- 此前 **AI 去广告与 AI 智能官替的失败都是静默的**：`runClean` 中 `aiDetectAdIndexes` 出错直接忽略；官替链路中 `aiExtractVideoInfo` / `aiPickPlay` 出错直接回退规则。用户勾选了 AI 但实际 AI 未参与时没有任何提示，无法分辨是「没填 Key / Key 错 / 分段超限 / 免费模型返回异常」。
- 典型场景：m3u8 分段数超过 `max_segments`（默认 300）时 AI 审核直接跳过，只靠规则识别，看起来就是「识别不出广告」。

### 2. 修复

- **`ParseResult` 新增 `ai_message` 字段**：`engine=ai` 时无论成败都回填状态——`AI 审核命中 N 段` / `AI 审核完成，未新增命中` / `AI 审核未生效: <具体原因>`；同步拼入 `message`，JSON 接口（`/api/clean/json`、`/api/clean/enhanced/json`）直接可见；
- **官替链路 AI 步骤透出失败原因**：`aiExtractVideoInfo` / `aiPickPlay` 改为返回 error，失败时在步骤中记录 `AI 未生效: <原因>`（未启用/未配置 Key/HTTP 状态码/返回无法解析/编号越界等），后台「官替解析」步骤列表一目了然；
- **清理无用代码**：移除 `aiDetectAdIndexes` 中无效的 codeHint 判断。

### 3. 免费模型试用排查路径

1. 后台「🧠 AI 大模型接入」选免费提供商（智谱 GLM-4-Flash）→ 填 Key → 💾保存 → 📡测试连接必须「连通成功」；
2. 「解析测试」选「去广告引擎=AI」解析 m3u8 → 看返回统计里是否带「引擎=AI · AI 审核命中/未生效: 原因」（分段超 300 时调大「AI 审核最大分段」）；
3. 「官替链路」勾「☑ AI 智能判定」解析官方链接 → 看步骤里 `ai_meta`（识别剧名/集数）与 `ai_rank`（AI 选最优资源）是否 ok，fail 时直接给出原因。

### 4. 验证与版本

- `go build` + `go vet` 通过；版本升级 `v0.6.27 → v0.6.28`。

---

## Go 分支 v0.6.27 (2026-09-12) — 免费 AI 大模型接入：0 成本试用（智谱 GLM-4-Flash 等）

> 用户诉求：先用网络上免费的模型试试效果（功能要求不高），后续再考虑用 AI 去插播/广告。

### 1. 免费方案一键配置（[main.go](file:///workspace/main.go)）

- **默认配置预填免费方案**：`ai/config.json` 与内置默认改为「智谱 GLM-4-Flash」（完全免费、手机号即开、国内直连、中文好），只需填 API Key 并勾选启用即可试用，无需再填接口/模型；
- **预设列表重排并标注免费**：`zhipu` 智谱 GLM（免费）、`siliconflow` 硅基流动（送2000万Token+9B以下模型永久免费）、`modelscope` 魔搭（免费2000次/天）、`baidu` 百度千帆（ERNIE-Speed 永久免费）、`groq` Groq（免费额度，需科学上网）置顶并标「免费」，其余付费/本地提供商后置；
- **后台面板加免费试用指引**：AI 大模型接入面板顶部新增推荐说明与注册入口提示。

### 2. 免费试用三步走

1. 任选一家免费平台注册拿 Key（智谱 open.bigmodel.cn 手机号即开）；
2. 后台「🧠 AI 大模型接入」→ 下拉选对应免费提供商 → 粘贴 Key → 💾 保存；
3. 📡 测试连接「连通成功」→ 勾选「启用 AI」/「AI 智能官替」→ 去「解析测试 / 官替链路」试用。

### 3. 验证与版本

- `go build` + `go vet` + 前端 JS 语法检查通过；版本升级 `v0.6.26 → v0.6.27`。
...
- `/api/clean`（及 `/api/clean/enhanced`）此前直接把 `buildFilteredM3U8` 生成的**源站绝对地址**（如 `https://v.lzcdn28.com/.../xxx.ts`）作为无广告 M3U8 抛给调用方；
- 播放器跨域请求这些源站 CDN 分片时，会被源站的跨域/反爬/Referer/IP 限制卡住（尤其服务端在公网、或被拉流方网络环境屏蔽），导致「过滤成功但播放器拉不到流 → 黑屏/无法起播」。
- 实测：`/api/clean?url=<lzcdn>&opt=aggresive` 解析 696 段、识别广告 9 段、保留 687 段——过滤逻辑正常，问题出在**返回地址指向源站而非本服务代理**。

### 2. 修复

- 新增 `requestProxyBase`（基于请求 Host 生成绝对代理前缀）与 `outCleanM3U8` 输出函数：
  - 把过滤后的无广告 M3U8 的**分片/密钥地址统一改写为 `/api/play?url=<源地址>`**，由本服务拉流转发（附带正确 UA + Referer，支持 Range 拖动）；
  - `/api/clean`、`/api/clean/enhanced` 两条「直接出 M3U8」的响应改走代理输出，保证返回结果任意播放器都能正常拉流播放；
  - `format=json` 仍然返回字段完整的源站结构化结果，供服务端二次分析/抓取源站绝对地址。
- 同步更新后台「HTTP 接口说明」与 API 列表中 `/api/clean` 的用法说明。

### 3. 验证与版本

- 复测报告 URL：`/api/clean?url=<lzcdn>&opt=aggresive` 返回 687 段、全部为 `http://<host>/api/play?url=…` 代理地址（源站直连 0 条）；分段请求返还 HTTP 200、拖动 `Range` 返还 HTTP 206，端到端可正常播放。
- `go vet` + 全量编译通过；版本升级 `v0.6.23 → v0.6.24`。

---

## Go 分支 v0.6.23 (2026-09-12) — 全量资源站样本去广告分析 + 修复「返回成功却无法正常播放」

> 用户诉求：按照现有的资源站，抓取每个资源站的随意（样本）链接，进行「去插播和广告」的分析与规则校验，并优化「结果返回成功但实际不能正常播放」的问题。

### 1. 全量资源站样本分析（67 个活跃站点 · 逐站抓样本 m3u8 实测）

对 67 个可用的资源站逐一抓取样本播放地址并跑完整去广告流程，量化「广告识别」与「播放安全」：

- **CDN 分类** 实测样本多集中在几大 CDN 家族（快车/闪电/牛牛/自然/锦发系、量子/lzcdn、猫眼/m3u8 等），多数站点无显式广告标签（`#EXT-X-CUE`/`#EXT-X-DATERANGE` 出现为 0）。
- **真实广告形态** 集中在：① 同目录重复短时长片段簇；② 中插明显的超短段簇（两侧被长正片夹住）；③ 少量 URL/文件名含 `ad`/`tvc` 等关键词。
- **「播不了」根因**（本次修复重点）：统一正片被启发式规则误判删除——
  - `cluster_repeat_duration`（同目录聚类去重）可把整部正片全部标记为广告 → 全片 361~3682 段删除、保留 0；曾致 16 个站点「成功返回但空列表」；
  - `enhanced_discontinuity_block` 把「分隔符型」DISCONTINUITY（快车/闪电/牛牛/无水印等 CDN 每个 ~2s 段前都放一个 `#EXT-X-DISCONTINUITY`，146/1293 段）当成成对广告块 → 单视频误删 42%+（505 段）；
  - `ultra_short` 对统一短段正片的单帧级短段过度剔除。

### 2. 规则优化与播放安全兜底（[main.go](file:///workspace/main.go)）

1. **新增 `applyAdSafetyRoof` 播放安全兜底总开关**——「宁可不删广告，不可播不了」：
   - 任何启发式检测把整部删除比例 >50%（或删空）时，判定为统一正片误伤，**回退全部低置信启发式标记**，仅保留高置信显式标记（URL 关键词 / 播放列表显式广告标签区间）；
   - 回退后若仍为空，则彻底保留全部片段（放弃过滤，保证能播）。
   - 分别接入 `/api/play` 播放代理（`cleanPlaylistOnce`）、`/api/audit` 广告核查、`/api/clean*` 去广告引擎（`runClean`）三条链路。
2. **关闭聚类去重误杀路径**：`cleanPlaylistOnce` 与 `auditParse` 改为 `detectAds(segs, false)`，禁用 `detectClusters`（其注释本就注明 PHP 版 v5.15.4 已默认为避误判关闭，Go 版此前被 aggresive 误开启）。
3. **`markDiscontinuityBlocks` 增加分隔符型保护**：
   - 若带 `DISCONTINUITY` 的片段占比 >5%，判定 DISCONTINUITY 为分段分隔符（非广告插入点）直接跳过该规则；
   - 候选广告块还需「块内平均时长 < 全片平均时长 × 0.6」才判，双保险避免统一短段正片被误删。
4. **效果**（67 站实测）：保留比例分布由「16 站清空 / 22 站 <30%」修复为「最低保留 89%，绝大多数 ≥95%，无任何站点被删空、无 >40% 误删」，**「成功返回但不能正常播放」消除**；真实广告（URL 关键词 / 中插超短簇 / 显式广告标签区间）仍被准确剔除。

### 3. 版本与验证

- `go vet` + 全量编译通过；以真实站点样本 m3u8 复测配额：无空列表、无误删过半；版本升级 `v0.6.22 → v0.6.23`。

---

## Go 分支 v0.6.22 (2026-09-12) — 资源站一键自动导入 + 资源站列表溢出显示优化

> 用户诉求：① 添加自动获取脚本——从链接自动抓取、解析、批量添加资源站，只需点一下即可；② 优化资源站列表「内容超出显示」的问题。

### 1. 资源站一键自动导入（[main.go](file:///workspace/main.go)）

- 新增 `POST /api/sites/import`（需登录）：body 支持 `{url}`（从链接抓取）或 `{text}`（粘贴文本）二选一，自动解析并批量添加，返回「解析/新增/跳过」统计；
- 解析器兼容 kdocs 表格文本（`序号|名：官网 | 采集：接口 | 备注`）与平铺（`名：官网 采集：接口 备注`）两种格式；自动去 HTML 标签、GBK 乱码兜底；
- 自动识别「停更 / 移除以下」分组标题，其后的站点行**不导入**（避免把死链/待移除站加进来）；已存在同名/同接口站点跳过（不覆盖用户已有配置）；
- 后台「资源站管理」新增两处入口：**「⚡ 一键从链接导入」**（粘贴含资源站列表的网址）+ **「📋 粘贴文本」**（kdocs 等表格文本直接粘贴），一次点击完成，并即时刷新列表。

### 2. 资源站列表溢出显示优化（[main.go](file:///workspace/main.go)）

- `.siterow` 增加 `min-width:0; flex-wrap:wrap`，站点名：`max-width:34%` + 超长省略号；备注：`.note` 弹性省略；按钮不收缩、不换行；
- `#siteList` 限宽 `max-width:100%`，窄屏/长文本不再横向溢出面板。

### 3. 版本与验证

- `go vet` + 全量编译通过；单元测试验证 kdocs 文本正确解析（量子/暴风/无尽名称+接口正确，「停更/移除以下」分组被跳过）；版本升级 `v0.6.21 → v0.6.22`。

---

## Go 分支 v0.6.21 (2026-09-12) — 清理失效资源站并同步 kdocs 权威资源站列表

> 用户诉求：删除所有失效资源站，并从 https://www.kdocs.cn/l/clVLUaNbX1Xq 的表格文本中提取资源站添加进来.

### 变更

- **删除失效资源站**：`resource_sites.json` 由 165 个站点清理为 81 个活跃站点，删除全部 102 个 `paused`（失效/自动屏蔽）站点，并按 kdocs「移除以下资源站」分组移除 `黑木耳`（跑路/封IP）。
- **同步 kdocs 权威列表**：新增 19 个站点（优质/无印/魔都/U酷/豆瓣/天涯/爱奇艺/索尼/牛牛/无尽/无尽（无水印）/新浪/极速/淘片/茅台/虎牙/金鹰/大众/ikun）；对 15 个同名站点更新采集接口为 kdocs 最新地址（如暴风→`by.bfzyapi.com`、猫眼→`api.maoyanapi.top`、百度→`api.apibdzy.com`、樱花→`m3u8.apiyhzy.com` 等）。kdocs「停更」「移除以下」两组站点不加入。
- 同步再生成内置默认配置 `sites_static.go`（与 `resource_sites.json` 一致），全新部署开箱即用。
- 版本升级 `v0.6.20 → v0.6.21`。

> 部署提示：已运行的服务优先读取磁盘上的 `resource_sites.json`，升级新版本后请一并更新服务器上的 `resource_sites.json` 或删除该磁盘文件让其回退到内置新列表。

---

## Go 分支 v0.6.20 (2026-09-12) — 修复 /api/jx* 接口不同链接返回相同结果（壳标题脏映射 + 官链真实标题解析增强）

> 用户诉求：`http://114.134.184.91:8080/api/jx/client?url=` 等接口出现问题——**不同链接返回一样的结果**，官方资源与 m3u8 后缀资源都是如此。

### 根因

官方视频页（尤其腾讯视频）是 JS 渲染壳，静态抓取 `<title>` 只有平台名「腾讯视频」。该壳标题此前被**自动学习**写成了到某剧目的映射（「腾讯视频」→具体剧目）。于是所有「抓不到真实剧名」的官方页都会命中同一映射、匹配到同一个资源 → **不同链接返回相同结果**。

### 1. 壳标题脏映射防护（[main.go](file:///workspace/main.go)）

- 新增 `isShellTitle`：识别平台展示名/平台首页通用词标题（「腾讯视频」「爱奇艺」「搜狐视频-高清视频在线观看」等）为「壳标题」，判定其不携带真实剧名；
- `autoLearnTitleMap` 防脏：官方剧名为空/是壳标题/可疑时**不再自动学习**映射，杜绝「某平台名→某剧名」这种污染源头；
- `titleMapCandidates` 匹配时**跳过壳标题作键的映射**，已写入的脏映射不再参与展开（同关不同剧页不再被拉到同一资源）；
- `loadTitleMaps` + 启动预载：加载时**自动清洗并删除壳标题作键的历史脏映射**，后台不再展示失效映射。

### 2. 腾讯真实剧名解析增强（[main.go](file:///workspace/main.go)）

- `fetchTencentVid` 支持从**路径**提取当前集 vid：`/x/page/{vid}.html`、`/x/cover/{专辑id}/{vid}.html`、旧式 `/x/cover/{vid}.html`（此前仅认 `?vid=` 参数）；
- `fetchVideoTitle` 对腾讯：只要 vid 可解析且静态标题为壳标题，就**强制用 getinfo 接口取正式片名**（此前仅在标题为空时尝试，壳标题会短路）；
- 壳标题/取不到标题时新增 **render-title/ 无头渲染兜底**（未部署时零开销立即返回），JS 渲染壳页可拿真实剧名。

### 3. 官链条路壳标题拦截（[main.go](file:///workspace/main.go)）

- `replaceOne` 标题仍为壳标题时明确返回「无法获取视频信息」，不再用通用关键词兜底搜索，避免跨剧错配。

### 4. 验证

- 本地模拟「腾讯视频→独剑九天」历史脏映射：修复前返回「独剑九天」，修复后正确返回「无法获取视频信息」，不再跨链接返回相同结果；
- 不同 m3u8 链接经 `/api/jx/client`、`/api/play` 返回各自独立内容（段数/地址均不同），无缓存串扰；
- `go vet` + 全量编译通过；版本升级 `v0.6.19 → v0.6.20`。
...
### 1. 资源站·失效站可折叠（[main.go](file:///workspace/main.go)）

- 资源站列表底部新增 `❌ 已失效/暂停（N）` **折叠区**（默认折叠，点击展开查看被屏蔽/失效站，含备注/编辑/删除）；
- 主列表只显示可用站（按启用/未启用分组），失效站自动归入折叠区，不再混在主列表；统计栏显示「已启用 / 可用 / 失效」。

### 2. 自动更新官方·一键映射（[main.go](file:///workspace/main.go)）

- 内置**各大官方平台默认链接**（腾讯/爱奇艺/优酷/芒果TV/哔哩哔哩/搜狐/PP）清单；
- 官方面板顶部新增「⚡ **一键映射**」按钮区：点击某个官方平台 → 后端**实时抓取**该链接标题 → 解析影视剧名 → **自动写入映射表**（无需输入链接）；
- 接口：`GET /api/platforms/links`（内置链接清单）、`POST /api/platforms/oneclick?key=<内置key>`（无脑映射）；
- 反爬防护：抓取标题命中「验证码/安全验证/访问异常/404」等或过短时**拒绝映射**并提示，避免脏数据进入专区。

### 3. 版本与验证

- 版本升级 `v0.5.5 → v0.5.6`；[README.md](file:///workspace/README.md) 同步 v0.5.6 更新日志；
- 验证：`CGO_ENABLED=0 go build -o mxgt-go .` 通过，`gofmt` 干净；冒烟测试登录 → links → oneclick → maps 贯通，badkey 正确报错。

---

## Go 分支 v0.5.5 (2026-09-09) — 资源站「搜索验证站」识别标注：不误判失效

> 用户诉求：资源站 100 多个，其中有些站有**搜索验证**（关键词搜索接口需验证/未开放，直接搜索会无结果或失败）。本版在批量检测中明确区分这类站点：搜索无命中但列表接口连通 → **仍判可用**（不误判失效），并在检测结果与站点列表中**标黄/备注提示**，便于一眼识别。

### 1. 搜索受限站识别标注（[main.go](file:///workspace/main.go)）

- 批量检测项新增 `search_limited` 标记：`probeSiteOne` 多探针词均无结果、但列表接口连通且返回有效数据时置位；
- 检测进度列表**橙黄标注**「⚠️ 可用(搜索受限)」：这类站靠接口连通性判定可用，不会被打入失效队列（自动置 active 而非屏蔽）；
- 检测完成后**常驻备注**：「搜索受限/需验证·仅接口连通」写入查询站 `note`，资源站列表一眼可辨；若后续正常搜索命中则自动清除该备注；
- 既不误判失效，也明确提示哪些站官替关键词搜索可能搜不到（不影响其它站解析）。

### 2. 版本与验证

- 版本升级 `v0.5.4 → v0.5.5`；[README.md](file:///workspace/README.md) 同步 v0.5.5 更新日志；
- 验证：`CGO_ENABLED=0 go build -o mxgt-go .` 通过，`gofmt` 干净。

---

## Go 分支 v0.5.4 (2026-09-09) — 官替映射「自动学习」：无需手动逐个添加

> 用户诉求：官替映射专区不想一条一条手动添加，希望能自动生成映射。本版在官替主流程中开启**自动学习**：每次官替解析成功命中资源站、且资源站剧名与官方剧名表述不同时，自动生成「官方剧名 → 资源站标准剧名」映射并保存，无需前端操作；映射被匹配逻辑双向往回展开，下次同类剧名直接命中。

### 1. 官替映射自动学习（[main.go](file:///workspace/main.go)）

- 新增 `autoLearnTitleMap`：官替成功后自动把「官方剧名(vi.BaseTitle) → 资源站标准剧名(parseVideoTitle 后的 base_title)」写入 `title_maps.json`，自动去重（相同 from→to 不重复生成）、同名不生成（避免冗余）、资源站拼接剧名自动取基础剧名；
- `replaceOne` 匹配成功处接入：返回字段新增 `auto_learn_map`（本次自动生成的映射）与 `auto_learn_count`（自动学习累计条数）；
- 前端官替结果状态行：命中后显示「🧠 自动生成映射…（累计 N 条）」+「🗺️ 查看映射表」快捷入口；官替映射专区标题提示「已开启自动学习，无需手动逐个添加，手动抓取表单仍可用」。

### 2. 版本与验证

- 版本升级 `v0.5.3 → v0.5.4`；[README.md](file:///workspace/README.md) 同步 v0.5.4 更新日志；
- 验证：`CGO_ENABLED=0 go build -o mxgt-go .` 通过；单测覆盖自动学习四类场景——表述不同→生成（独剑九天→剑出九霄、武林外传→武林外传传奇）、同名→不生成、重复→不重复生成，全部通过。

---

## Go 分支 v0.5.3 (2026-09-09) — 资源站检测去误判 + 官替「自动更新官方」平台配置

> 用户诉求：资源站检测一次性失效 119 多个不可能全部失效（单探针词误判）；官替映射专区需要「自动更新官方」——有顺序、平台、域名、映射字段（影视剧名+剧集集数）可从真实链接自动获取并映射到对应区域，编辑项含平台名称/域名/URL 匹配正则/标题选择器/优先级。

### 1. 资源站检测去误判（[main.go](file:///workspace/main.go)）

- 新增 `probeSiteOne` 综合探测：**多探针词**（「爱情/庆余年/电视剧」）任一命中即判可用，避免单词语义或站点内容覆盖不足导致的整批误判失效；
- 搜索均无结果时**探测列表接口连通性**（`?ac=videolist&limit=5`），接口返回有效数据即判「接口正常（搜索无命中）」；
- 仅当 HTTP 错误/超时/解析失败才置失效；「🧹 检测并屏蔽失效站」按钮文案同步更新为多探针说明，前端进度条展示逐站探测结果。

### 2. 官替「自动更新官方」平台配置（[main.go](file:///workspace/main.go)）

- 新增 `OfficialPlatform` 配置数据结构（`official_platforms.json` 持久化）：**顺序（优先级）/平台名称/域名/URL 匹配正则/标题选择器/优先级/启用/备注**，内置 7 平台默认配置（腾讯/爱奇艺/优酷/芒果TV/哔哩哔哩/搜狐/PP）；
- `matchOfficialPlatform` 按优先级顺序匹配 URL：**域名包含 + URL 正则（可选）**，先匹配到即用；`detectPlatform` 优先使用用户配置，未配置时回退内置平台提示；
- `fetchVideoTitleWithSelector` 支持**自定义标题选择器**（正则第 1 捕获组）从真实页面提取标题，空则回退默认 `og:title`/`<title>`；
- 新增 REST API：`GET /api/platforms` 列表、`POST /api/platforms/add` 添加、`POST /api/platforms/update` 编辑（按旧平台+旧域名定位，校验正则合法性）、`POST /api/platforms/delete` 删除；
- `GET /api/platforms/fetch?url=<真实官方链接>` **自动更新官方**：按优先级匹配平台 → 标题选择器提取 → `parseVideoTitle` 自动解析「影视剧名 + 剧集集数」两个映射字段返回；
- 后台「🛰️ 自动更新官方」面板：配置列表（顺序/平台/域名/URL 正则/标题选择器/优先级/启用/操作）+ 添加/编辑/删除/启停 + 粘贴真实链接一键获取字段；获取成功后**自动映射到「官替映射专区」表单**（from=影视剧名、平台自动填入），可直接「➕ 添加映射」落到映射表对应区域。

### 3. 版本与验证

- 版本升级 `v0.5.2 → v0.5.3`；[README.md](file:///workspace/README.md) 同步 v0.5.3 接口与更新日志；
- 验证：`CGO_ENABLED=0 go build -o mxgt-go .` 通过；API 冒烟测试（登录 → platforms 列表/添加/更新/删除 → fetch 未匹配提示 → maps 列表）全部通过。

---

## Go 分支 v0.5.2 (2026-09-09) — 修复剧名/集数拆分 + 内置 7 种官方平台字段提取

> 用户诉求：官方标题「【腾讯视频】 独剑九天 01」被整体当作剧名（集数未拆分），导致搜索关键词错误、映射专区字段提取错误；需要内置 7 种官方平台（腾讯/爱奇艺/优酷/芒果TV/哔哩哔哩/搜狐/PP）的映射，从真实链接获取真实字段，提高准确率。

### 1. 修复剧名/集数字段拆分（[main.go](file:///workspace/main.go)）

- 新增 `platformTagRe`：**内置 7 种官方平台**的剧名标签/后缀清理规则（`【腾讯视频】`、`_爱奇艺`、`-优酷`、`芒果TV`、`哔哩哔哩/bilibili`、`搜狐视频`、`PP视频` 等），从官方标题提取真实剧名字段；
- `parseVideoTitle` 增强：**剧名 + 末尾数字 → 拆分集数字段**，兼容「独剑九天 01」「独剑九天01」（剧名+空格/分隔符+数字优先；紧跟 2 位数字次之，避免误拆「流浪地球2」类续作剧名）；
- 验证（单测）：`「【腾讯视频】 独剑九天 01」→ 剧名=独剑九天、集数=1`；`「独剑九天01」→ 集数=1`；`「剑出九霄 第12集」→ 集数=12`（多位数正确）；`「独剑九天_爱奇艺」→ 剧名=独剑九天`；`「流浪地球2」→ 不再误拆`；
- 修复后搜索关键词正确生成「独剑九天 第1集 / 独剑九天 1 / 独剑九天」，映射专区 from 字段只含剧名。

### 2. 内置 7 种官方映射

- 代码内置 7 平台字段提取规则（识别 + 平台标签清理 + 剧名集数拆分 + 集数表述兼容）；
- `title_maps.json` 首次创建时自动落盘内置示例映射（腾讯视频「独剑九天→独剑九天」，用户可改/删）；
- 后台映射专区标题提示「已内置 7 种官方平台字段提取」。

### 3. 版本与验证

- 版本升级 `v0.5.1 → v0.5.2`；[README.md](file:///workspace/README.md) 同步 v0.5.2 更新日志；
- 验证：`go vet` / `CGO_ENABLED=0 go build -o mxgt-go .` 通过。

---

## Go 分支 v0.5.1 (2026-09-09) — 官替映射自动填充 + 内置/外置播放

> 用户诉求：官替映射只要输入官方链接就自动映射剧名字段和剧集字段（部分字段直接包含当前剧集）；官替增加内置播放或外置播放（需加跨域、url 不硬编码）。

### 1. 官替映射自动填充（[main.go](file:///workspace/main.go)）

- `/api/maps/fetch` 返回增强：新增 `title_raw`（官方原始标题，可能直接包含当前剧集如「独剑九天01」「独剑九天 第01集」）与 `episode_raw`（原始剧集字段表述）；
- 后台映射专区「🔍 从链接抓取」后**自动映射**：剧名字段（`from`=官方剧名）与平台字段自动填入表单，同时显示拆出的「剧名字段 + 剧集字段」；
- 「➕ 直接添加映射」一键提交：`to`（资源站标准剧名）留空时默认等于官方剧名，减少手动输入。

### 2. 官替内置 / 外置播放

- `/api/replace` 返回新增 `play_url`（内置播放地址）与 `external_url`（外置播放页地址）；
- **内置播放**：后台官替结果「▶ 内置播放」按钮，直接在当前页面内嵌播放器播放（复用现有 hls.js/原生播放器，支持 mp4 直链）；
- **外置播放**：新增独立播放页 `GET /player?url=<去广告直链>&title=<剧名>`（开放、无需登录），「↗ 外置播放」按钮新窗口打开，hls.js 多 CDN 兜底 + iOS 原生 HLS + mp4 直链原生播放；
- **跨域**：`/player` 响应与播放分片请求均带 CORS 头（`withCORS` 全局中间件），hls.js 请求分片时携带 `Origin` 头；
- **URL 不硬编码**：`play_url` / `external_url` 均由 `playURL` 基于请求 `Host` 动态拼接（`http(s)://<当前Host>/api/clean|/player?…`），无任何硬编码域名/IP。

### 3. 版本与验证

- 版本升级 `v0.5.0 → v0.5.1`；[README.md](file:///workspace/README.md) 同步 v0.5.1 更新日志；
- 验证：`go vet` / `CGO_ENABLED=0 go build -o mxgt-go .` 通过。

---

## Go 分支 v0.5.0 (2026-09-09) — 官替映射专区：解决官方剧名/剧集与资源站表述不同导致的匹配失败

### 官替映射专区（映射不同官方与资源站的剧名对应关系）

> 用户诉求：官替获取到官方信息，但搜不到/匹配不到正确剧集（如「独剑九天 01」）；官方剧名和剧集表述与资源站不同导致无法获取或获取不到；不同官方平台映射不同，需要映射专区从真实链接中获取并添加映射。

#### 1. 映射数据结构与持久化（[main.go](file:///workspace/main.go)）

- 新增 `TitleMap`（from=官方剧名 → to=资源站标准剧名，含平台/备注）与 `TitleMaps` 配置；
- 持久化到可执行文件旁的 `title_maps.json`（独立文件，不影响资源站配置），提供 `load/save` 与并发锁。
...
- `GET /api/maps/fetch?url=<真实官方视频页>`：**从真实链接抓取官方平台/剧名/集数**，识别腾讯/爱奇艺/优酷/芒果TV/哔哩哔哩/搜狐/PP 视频，抓 `og:title`/`<title>`（腾讯无标题走 `getinfo` 兜底），解析出 `base_title` 与 `episode_num` 供用户直接填入映射表单。

#### 3. 匹配逻辑增强

- `titleMapCandidates`：搜索与匹配时对官方剧名与资源站剧名**双向展开映射变体**，命中映射后自动把别名加入候选，提高不同表述的命中率；
- `findBestMatch` 使用映射变体两两比较取最高相似度；搜索关键词自动包含「剧名+第N集 / 剧名+N / 剧名」三档，优先精确匹配集数；
- `episodeNumOfPlayItem` 兼容「第01集 / EP01 / E1 / 01 / 独剑九天01 / 1」等多种集数表述。

#### 4. 后台 UI（映射专区面板）

- 后台新增「🗺️ 官替映射专区」面板：粘贴真实官方视频页链接 →「🔍 从链接抓取」自动获取官方平台/剧名/集数 →「📥 填入下方表单」→ 填资源站标准剧名 →「➕ 添加映射」；
- 映射表格展示 平台/官方剧名/标准剧名/备注，支持逐条删除；页面加载自动拉取映射列表。

#### 5. 修复：多位数集数解析错误

- 修复 `cnToNum` 对纯阿拉伯数字逐字解析的缺陷（如「第12集」被解析成 2），现直接按整数转换（"01"→1、"12"→12、"100"→100），保证超过 9 集的剧集正确匹配与取集。

### 版本与验证

- 版本升级 `v0.4.9 → v0.5.0`；[README.md](file:///workspace/README.md) 同步 v0.5.0 更新日志；
- 验证：`go vet` / `CGO_ENABLED=0 go build -o mxgt-go .` 通过。

---

## Go 分支 v0.4.9 (2026-09-09) — 资源站支持编辑（接口/官网/备注/优先级/改名）

### 资源站编辑接口

> 用户诉求：资源站不能编辑接口。

#### 1. 新增编辑接口（[main.go](file:///workspace/main.go) `handleSiteUpdate`，参考 PHP `updateSite`）

- `POST /api/sites/update`：body `{name: 定位原名, api_url, site_url, note, priority, enabled, new_name?}`；
- `name` 精确 + 忽略大小写兜底定位站点；可更新采集接口 / 官网 / 备注 / 优先级 / 启停；
- `new_name` 可选改名（自动查重，重名拒绝）；接口非空时校验 `http(s)://` 开头；
- 修改实时落盘 `resource_sites.json`；`/api/stats` 埋点记录。

#### 2. 后台 UI

- 站点每行新增「✏️ 编辑」按钮（详情与删除之间），行内展开编辑表单：名称 / 接口 / 官网 / 备注 / 优先级 + 「💾 保存」；
- 表单预填当前值，保存成功 alert 提示并自动刷新列表。

#### 3. 版本与验证

- 版本升级 `v0.4.8 → v0.4.9`；[README.md](file:///workspace/README.md) 同步 v0.4.9 更新日志；
- 验证：`go vet` / `CGO_ENABLED=0 go build -o mxgt-go .` 通过；接口实测：编辑接口+备注成功并落盘、改名（西瓜→西瓜影视→西瓜）成功、非法接口（ftp://）拒绝、不存在站点返回「资源站不存在」；浏览器实测：编辑按钮可见、表单正确预填、保存后备注更新、再次展开显示最新值、布局正常。
...
#### 1. 根因（[main.go](file:///workspace/main.go) `main()`）

- 原实现 `ListenAndServe` 一旦返回 `bind: address already in use` 即 `log.Fatal` 退出；
- 场景：在线更新重启时旧进程未完全释放端口、端口处于 TIME_WAIT、或端口被其它程序占用 → 新版进程启动即绑定失败直接退出，服务起不来。

#### 2. 修复

- 启动改为循环监听：端口被占用时打印「端口被占用，2s 后重试(k/30)」，`time.Sleep(2s)` 后重试，最多 30 次；端口释放后自动绑定成功接续启动；
- 非端口类致命错误仍 `log.Fatal` 并给出明确日志；优雅关闭（`http.ErrServerClosed`）正常退出；
- 与 v0.4.1「更新重启先释放端口再启动」配合，进一步减少在线升级失败的窗口。

#### 3. 版本与验证

- 版本升级 `v0.4.4 → v0.4.5`；[README.md](file:///workspace/README.md) 同步 v0.4.5 更新日志；
- 验证：`go vet` / `CGO_ENABLED=0 go build -o mxgt-go .` 通过；实测：先占 `:8100` 再起 v0.4.5 → 原会 `bind: address already in use` 直接退出；现改为持续重试保持存活，释放端口后 `healthz` 返回 `ok v0.4.5` 接续启动成功。
