...
> 压字广告/水印/滚动字幕"各种位置都有"，用**视觉大模型识别**：截取当前视频帧发给支持"看图"的多模态模型，返回画面内所有压字广告所在归一化矩形，播放器自动在这些位置叠加遮罩（顶/底/四角/中段都能盖）。支持 OpenAI 兼容 / Anthropic / Gemini 协议。接口 `POST /api/overlay/detect`；播放页新增「🎯 自动识别压字广告位置」按钮。**前提**：AI 大模型需配支持看图的多模态模型（如 GLM-4V / gpt-4o / Qwen-VL）。版本 `v0.6.34 → v0.6.35`。
