...
- 已运行服务**优先读取磁盘 `resource_sites.json`**，升级新版本后如需用新列表，请一并更新该文件或删除它回退到内置列表（见下方「部署」升级注意）。
...
> **🆕 升级注意（资源站列表）**：服务**优先读取可执行文件旁的磁盘 `resource_sites.json`**（后台启停/自动导入都会改它）。升级后若想让服务使用**新版内置列表**（含新站点/最新接口），请**一并更新服务器上的 `resource_sites.json`**；或直接**删除**磁盘上的该文件，让服务回退到内置默认列表（首次运行会自动重新生成）。仅替换二进制不会自动替换该文件。
...
## v0.6.35 (2026-09-13) — AI 视觉识别压字广告位置，自动遮挡任意位置

> 压字广告/水印/滚动字幕"各种位置都有"，用**视觉大模型识别**：截取当前视频帧发给支持"看图"的多模态模型，返回画面内所有压字广告所在归一化矩形，播放器自动在这些位置叠加遮罩（顶/底/四角/中段都能盖）。支持 OpenAI 兼容 / Anthropic / Gemini 协议。接口 `POST /api/overlay/detect`；播放页新增「🎯 自动识别压字广告位置」按钮。**前提**：在「AI 大模型接入」的**🖼️ 视觉识别模型**填入支持看图的多模态模型（本配置已预填免费的智谱 `glm-4v-flash`；也可用 `glm-4v-plus` / gpt-4o / Qwen-VL），去广告/官替仍用下方免费文本模型 `glm-4-flash`，互不影响。版本 `v0.6.34 → v0.6.35`。

---

## v0.6.34 (2026-09-12) — 压字广告遮挡：画面内水印/压字/滚动字幕前端遮挡

> 烧录进视频像素的压字广告（如「本片由 69794.com 赞助发布…」）没有独立片段可删，无法靠 m3u8 去广告/区间跳过去除，改为**前端遮挡**：半透明遮罩条盖住顶部/底部固定水印与滚动字幕条 + 可选放大裁边把边缘压字挤出画面。浏览器端实时完成、不转码不卡顿。
> **使用**：后台新增「🧱 压字广告遮挡」面板（`overlay_masks.json` 全局配置：总开关/顶部条/底部条高度%/颜色/透明度/放大倍数），保存后所有播放器即时生效；单链接可用参数临时覆盖：`/player?url=…&overlay=on|off` 或 `overlay=top:10,bottom:8,zoom:1.1,opacity:0.5,color:000`。接口：`GET /api/overlay`（播放器读）/ `POST /api/overlay/save`（需登录）。版本 `v0.6.33 → v0.6.34`。

---

## v0.6.33 (2026-09-12) — AI 去广告更彻底 + 播放清单 VOD 秒拖秒播

> 修复「调用 AI 去除，部分插播、广告字幕、滚动字幕未完全去除」并支持播放链接秒拖秒播。
> **AI 去广告更彻底**：① 播放链路 `/api/play` 此前仅规则引擎、根本未走 AI，现改为「播放即 AI 去广告」（AI 已启用则自动对清单做一次语义审核，标记插播/滚动字幕/广告字幕等规则漏检项；AI 调用失败静默回退规则引擎，不影响播）；② AI 判定占比上限由 20% 放宽到 50%（原会把插播+字幕多的流当「幻觉」整批丢弃）；③ 连续广告块长度上限由固定 20 段放宽到全片 1/3；④ AI 标记 `ai_model` 升级为高置信，不被启发式误伤批量回退；⑤ 识别提示词扩展覆盖插播/前贴/中插/后贴/滚动字幕/广告字幕/角标水印/预告片。
> **秒拖秒播**：过滤输出清单新增 `#EXT-X-PLAYLIST-TYPE:VOD` + `#EXT-X-INDEPENDENT-SEGMENTS`，播放器据此开启完整随机 seek，拖动拖动即时定位（不再等待整清单缓存）。版本 `v0.6.32 → v0.6.33`。
...
> AI 去广告 + AI 智能官替「两个都要」，同时支持选择市面上的所有 AI 大模型接口。① 新增 12 个主流大模型提供商预设（OpenAI 兼容一大类：ChatGPT/DeepSeek/Kimi/智谱GLM/通义千问/腾讯混元/百度千帆/硅基流动/Ollama/自定义中转 + Anthropic Claude + Google Gemini），支持三大请求协议（openai/anthropic/gemini），`aiChatCompleteEx` 统一分发，AI 去广告与 AI 智能官替共用。② 后台新增「🧠 AI 大模型接入」面板：提供商下拉（选择自动填接口+默认模型）、接口/Key/模型/超时/分段/两条提示词、启用 AI + AI 智能官替两开关、💾保存（POST /api/ai/config）+ 📡测试连接（POST /api/ai/test）；Key 不回传明文。版本 `v0.6.25 → v0.6.26`。
...
## v0.6.23 (2026-09-12) — 全量资源站样本去广告分析 + 修复「返回成功却无法正常播放」

> 抓取 67 个活跃资源站样本 m3u8 逐站实测后发现：启发式去广告会误把「统一正片」当广告整片删除（聚类去重曾致 16 站返回空列表、DISCONTINUITY 分隔符型 CDN 误删 42%+），导致「成功返回但不能正常播放」。新增**播放安全兜底 `applyAdSafetyRoof`**（检测到删除比例 >50% 或删空时回退低置信标记，宁可不删广告也要能播，接入 `/api/play`、`/api/audit`、`/api/clean*` 三链路），关闭聚类去重误杀路径，并给 `markDiscontinuityBlocks` 增加分隔符型保护。实测保留比例从「16 站清空/22 站 <30%」修复为「最低 89%、绝大多数 ≥95%、无空列表」。版本 `v0.6.22 → v0.6.23`。

---

## v0.6.22 (2026-09-12) — 资源站一键自动导入 + 资源站列表溢出显示优化

> ① 新增 `POST /api/sites/import` 一键自动导入：后台「资源站管理」粘贴链接（从链接抓取）或粘贴文本（支持 kdocs 表格文本）→ 自动解析批量添加，识别并跳过「停更/移除以下」分组。② 优化资源站列表溢出显示：站点名/备注超长省略、按钮不换行、列表限宽。版本 `v0.6.21 → v0.6.22`。

---

## v0.6.21 (2026-09-12) — 清理失效资源站并同步 kdocs 权威资源站列表

> 删除所有失效资源站，并同步 https://www.kdocs.cn/l/clVLUaNbX1Xq 表格中的资源站。`resource_sites.json` 由 165 → 81 个活跃站点：删除全部 102 个失效(paused)站点 + `黑木耳`（kdocs 标注移除），新增 19 个 kdocs 站点，更新 15 个同名站点的采集接口为 kdocs 最新地址。同步再生成内置默认 `sites_static.go`。版本 `v0.6.20 → v0.6.21`。

> 部署提示：已运行的服务优先读取磁盘上的 `resource_sites.json`，升级后请一并更新服务器上该文件，或删除它让其回退到内置新列表。

---

## v0.6.20 (2026-09-12) — 修复 /api/jx* 接口不同链接返回相同结果（壳标题脏映射 + 官链真实标题解析增强）

> `http://114.134.184.91:8080/api/jx/client?url=` 等接口出现问题：**不同链接返回一样的结果**（官方资源与 m3u8 后缀资源）。根因是官方 JS 壳页标题「腾讯视频」被自动学成了到某剧目的脏映射，抓不到真实剧名的官方页全部命中同一映射→同一资源。

### 更新内容（[main.go](file:///workspace/main.go)）

- **壳标题脏映射防护**：新增 `isShellTitle` 识别平台展示名/首页通用词标题；`autoLearnTitleMap` 对壳标题**不再自动学习**映射；`titleMapCandidates` 匹配时**跳过壳标题作键的映射**；`loadTitleMaps` 加载时**自动清洗并删除壳标题作键的历史脏映射**；
- **腾讯真实剧名解析增强**：`fetchTencentVid` 支持从**路径**提取当前集 vid（`/x/page/{vid}`、`/x/cover/{专辑id}/{vid}`、旧式 `/x/cover/{vid}`）；`fetchVideoTitle` 对腾讯在壳标题时**强制用 getinfo 接口取正式片名**；壳标题/取不到标题时新增 **render-title/ 无头渲染兜底**（未部署时零开销立即返回）；
- **官链条路壳标题拦截**：`replaceOne` 标题仍为壳标题时明确返回「无法获取视频信息」，不再用通用关键词兜底搜索，避免跨剧错配。
- 版本升级 `v0.6.19 → v0.6.20`。

### 验证

- 本地模拟「腾讯视频→独剑九天」历史脏映射：修复前返回「独剑九天」，修复后正确返回「无法获取视频信息」，不再跨链接返回相同结果；
- 不同 m3u8 链接经 `/api/jx/client`、`/api/play` 返回各自独立内容（段数/地址均不同），无缓存串扰；`go vet` + 全量编译通过。

---

## v0.6.19 (2026-09-11) — 官方自动映射修复：腾讯/爱奇艺/B站/搜狐/PP 全部可取真实剧名

> 一键映射/自动抓取在多个官方平台失败（JS 渲染壳只有平台标题、B 站反爬「验证码/出错啦」、搜狐 GBK 乱码、PP 链接失效、爱奇艺拿不到剧名）。本版修复全部平台标题提取，并支持官替映射专区**自定义字段提取**。

### 更新内容（[main.go](file:///workspace/main.go)）

- **哔哩哔哩走公开 API**：视频页对数据中心 IP 反爬（412/「出错啦!」），新增 `fetchBilibiliTitle` 直接调 `api.bilibili.com/x/web-interface/view?bvid=` 取真实标题，无需登录；
- **爱奇艺改用 /adv/ SEO 静态页**：PC 播放页为纯 JS 壳（`<title>` 只有平台名），改抓 `iqiyi.com/adv/v_xxx.html` 内嵌 JSON「"name":"剧名第N集"」取真实剧名，decode+baseinfo 接口保留兜底；
- **搜狐乱码修复**：新增编码感知抓取（`fetchPageBytes`/`fetchPageBytesDecoded`），从 Content-Type / `<meta charset>` 探测字符集，GBK/GB2312 页面经 iconv 转 UTF-8；标题含乱码替换符（�）时自动按编码重抓，避免脏数据；
- **PP 视频换内置链接 + 无头渲染兜底**：旧链接失效（567 反爬），更新为当前可访问的具体视频页，一键映射优先走 render-title/ 无头 Chromium 渲染取真实剧名；
- **反爬/壳页/乱码标题拦截**：`suspiciousTitle` 增强——GBK 乱码（U+FFFD）、平台首页/壳页通用标题（如搜狐首页「综合视频网站/正版高清视频在线观看…」）命中即拒绝映射，避免脏数据进专区；
- **渲染失败自动回退**：一键映射中无头渲染结果若为反爬/出错页（如 B 站「出错啦!」）自动回退静态抓取 + 公开 API，不再卡死；
- **官替映射专区自定义字段提取**：`/api/maps/fetch` 新增 `selector` 参数（页面字段提取正则，第 1 捕获组为标题/字段值，如 `"name":"([^"]+)"`），前端新增「自定义提取字段」输入框，页面结构变化时无需改代码即可提取；
- **剧名后缀剥离增强**：`siteSuffixWords` 补充「音乐视频/聚力视频/原PPTV聚力视频/蓝光/超清」等，PP 视频「林则徐_电影_在线观看-PP视频-原PPTV聚力视频」→「林则徐」、搜狐「…-音乐视频」正确剥离；
- 版本升级 `v0.6.18 → v0.6.19`。

### 验证

- `go build` 通过（`main.go` + `sites_static.go`）；render-title 无头渲染实测：腾讯「金色_01」/ 爱奇艺「生逢其时」/ 搜狐「永远不要忘记的就是你的初心by孟美岐」/ PP「林则徐」全部解析成功；
- 一键映射 7 平台全通：腾讯「金色」/ 爱奇艺「生逢其时」/ 优酷「云游纪 (10)」/ 芒果TV「御廷谣」/ B站「Never Gonna Give You Up - Rick Astley」/ 搜狐「永远不要忘记的就是你的初心by孟美岐」/ PP「林则徐」；
- 反爬拦截验证：搜狐首页（平台壳页标题）、腾讯 cover 壳页（仅「腾讯视频」）均被拒绝映射，不再产生脏数据。

---

## v0.6.18 (2026-09-11) — API 调用方式标注客户端/服务端类型与用途

> 前台「🔌 API 调用方式」与后台「📚 HTTP 接口说明」为每个接口标注**调用类型**与**用途**：
>
> - 类型徽章：🟢 **客户端**（播放器/影视App/TVBox/盒子直连）、🟣 **服务端**（服务器间/二次处理）、🟠 **后台管理**（需登录）、⚪ **通用**（客户端与服务端皆可）；
> - 前台每条接口新增「用途：」说明行，明确该接口给谁用、怎么用（如 `播放代理` 标注「客户端：播放器直接播放本服务拼接的代理地址」）；
> - 后台表格新增「类型」列并补充用途说明，管理类接口统一标注「（需登录）」。

### 更新内容（[main.go](file:///workspace/main.go)）

- 前台 `frontPageHTML`：
  - `APIS` 数组每项新增 `t`（类型：client/server/admin/both）与 `u`（用途说明）字段；
  - 新增 `badgeOf(t)` 徽章渲染函数 + `.api-badge.{client,server,admin,both}` 彩色徽章样式 + `.api-meta .use` 用途样式；
  - `renderApis` 渲染标题后追加类型徽章、描述下追加「用途：」行；
- 后台 `adminPageHTML`：HTTP 接口说明表格增加「类型」列，全部接口标注类型 + 用途 + 是否需登录；
- 版本升级 `v0.6.17 → v0.6.18`。

### 验证

- `go vet` / `go build` 通过；
- 本地起服实测：前台渲染 19 个接口卡片，类型徽章与用途行均正确输出；Node 校验 `APIS` 数组分类：客户端 3 / 服务端 5 / 后台管理 7 / 通用 4。
...
>
> - **旧进程释放端口更快**：`Shutdown` 超时 6s → 2s（listener 立即关闭，活跃连接最多等 2s），端口轮询 8s → 3s；
> - **新进程端口绑定快速重试**：端口被占用时重试间隔 2s → 300ms（上限 30 次 → 100 次），旧进程释放端口后新进程几乎立即绑定成功，不再出现「重启后长时间卡在端口占用重试」；
> - 新进程启动后等待 200ms → 300ms 再退出，确保新进程完成绑定初始化再收尾。

### 更新内容（[main.go](file:///workspace/main.go)）

- `replaceAndRestart`：Shutdown 短超时 2s + `waitPortFree` 限时 3s，注释同步说明「重启等待时间过长」修复；
- `main` 绑定循环：端口占用重试 `2s×30次` → `300ms×100次`；
- 版本升级 `v0.6.15 → v0.6.16`。

### 验证

- `go vet` / `go build` 通过；
- 本地实测：两个实例争抢同一端口时，第二实例日志稳定输出「300ms 后重试」，首个实例释放端口后第二实例立即绑定成功并正常响应 `/healthz`。
...
> `/api/jx` 成功时 `msg` 由固定 `"ok"` 改为与 `url` 一致（同为可播放/去广告地址，经请求 Host 动态拼接），兼容部分影视 App / TVBox 从 `msg` 取播放链接的调用方；失败仍返回错误描述。`code`/`url` 等其它字段不变。

### 更新内容（[main.go](file:///workspace/main.go)）

- `handleJX` 成功分支：`resp["msg"] = resp["url"]`（与 url 一致）；同步更新接口注释；
- `recordCall` 记录随之写入可播放地址（仅日志，无副作用）；
- 版本升级 `v0.6.13 → v0.6.14`。
...
- `handleJX` 成功分支 `resp["code"]` 由 `1` → `200`；同步更新接口注释（`code:0/200`，200=成功、0=失败）；
- 前端无任何逻辑依赖 `code===1`，纯接口语义调整，零回归；
- 版本升级 `v0.6.11 → v0.6.12`。
...
## v0.6.11 (2026-09-11) — 修复「抓取失败: HTTP 404」：源站过期死链自动切换线路 + 明确报错指引

> 报错 `{"message":"抓取失败: HTTP 404 from https://vip.ffzy-plays.com/20260821/57276_ccCC450c/index.m3u8"}`。排查确认：该 `.m3u8` 是资源站每次「解析」动态生成的**带时效地址**（路径含日期分区 `20260821`，距今已过期），源站 CDN 已把文件删除，我用正确 UA/Referer 对其所有 vip 镜像域名实测均为源出口 nginx 404——**这是源站死链，任何本地代码都无法让已被源站删除的文件复活**，只能重新解析/采集拿到新地址。本次做两件事：① `/api/replace` 官替命中后若所选线路死链，**自动探测并切换到该资源其它线路/其它命中站点**（全部失效才回退原地址，正常链路零影响）；② 集中为死链报错给出可操作的明确提示。

### 更新内容（[main.go](file:///workspace/main.go)）

- **➕ 死链自动切换** `pickReachablePlayURL` + `m3u8Reachable`：
  - `m3u8Reachable` 用 `Range: bytes=0-0` 快速探测（源站 2xx→可用，4xx→死链），短超时，网络异常按不可用处理；
  - `pickReachablePlayURL`：优先当前命中视频的记录（先目标集数、再任意），失效则探测其它搜索命中视频（其它站点/线路，最多 4 条）；全部失效回退首个候选——**与原行为一致，不引入回归**；
  - `/api/replace` 选择播放地址时改用它，`ad_skip_url/play_url` 落到可达线路；
- **➕ 死链报错集中提示** `client.fetch`：源站返回 404/410 时，错误信息追加「（该播放地址已失效/过期，源站返回404：请从资源站重新解析或重新采集获取新的有效地址）」——覆盖 `/api/play`、`/api/clean`、`/api/audit`、官替取元等所有抓取入口；
- 版本升级 `v0.6.10 → v0.6.11`。

### 验证

- 实测失效地址 `vip.ffzy-plays.com/20260821/57276_ccCC450c/index.m3u8`：`m3u8Reachable=false`（正确判定为死链）；
- `/api/play`/`/api/clean` 请求该地址 → 新增 404 失效提示文案；
- `go vet` / `go build` 通过。
...
- **➕ 广告核查接口 `/api/audit`**（开放）：
  - 解析 m3u8 → 同一套增强去广告引擎（`detectAds` + `enhancedDetectAds` + 成对 DISCONTINUITY 块）→ 输出「保留段（过滤后时间轴）+ 规则已删广告段」；
  - 不连贯标记：`#EXT-X-DISCONTINUITY` 拼接点、时长 < 2s 偏短、与前后邻段均值 >2 倍 / <1/2 的时长突变——**每一个不是连贯的片段都列出**；
  - 时间轴为过滤后时间轴，与播放器一致，标记区间不错位；
- **🖥 后台 `🔍 广告核查` 面板**：
  - 输入 M3U8 + 视频标识（剧名 第N集，空=全局）→ 列出所有可疑片段（时间轴/时长/可疑原因/片段地址）；
  - `▶ 播放核对`：主播放器加载整片并定位到可疑段时间轴（可前后拖动看上下文，避免孤立播放误判）；
  - `⛔ 标广告`：确认后 POST `/api/skip/add` 写入跳过区间（type=sponsor），该视频播放时自动跳过；
  - `✅ 正常`：本次核查会话隐藏该行；下方 `🗑 规则已删广告段` 折叠区展示规则自动删除的广告段供抽查；
- 修复：核查面板移除冗余独立视频元素，统一走主播放器核对（避免核查播放与正式播放状态互扰）；
- 版本升级 `v0.6.8 → v0.6.9`。

### 验证（真实源 `vip.ffzy-play5.com/20221105/1107_99acb232`）

- `/api/audit` 输出：原始 318 段 · 规则已删广告 5 段 · 保留 313 段 · 可疑 81 处（拼接点/偏短/时长突变均已列出，含 5:59 周边）——规则判广告的 5 段零残留，其余可疑段供人工抽查；
- `go build` 通过。

---

## v0.6.8 (2026-09-10) — 新增成对 DISCONTINUITY 广告块检测，识别「插入点→短块→恢复点」漏检广告

> 实测源 `vip.ffzy-play5.com` 视频 5:59 出现广告（游戏广告"别再羡慕别人暴富"）：该广告在 m3u8 中是**独立分片**（段 89-93，约 20 秒），由**成对 `#EXT-X-DISCONTINUITY`** 标记（插入点段89 / 恢复点段94），但此前规则只按时长/URL 检测，这段时长 2-6.4s、无广告关键词 → 漏检未去除。本次新增**成对 DISCONTINUITY 广告块检测**：两个 DISCONTINUITY 之间夹 1-12 段且总时长 ≤ 120s 判为广告块（片头拼接/正片大段切分通常远大于此，避免误伤）。

### 更新内容（[main.go](file:///workspace/main.go)）

- **➕ 成对 DISCONTINUITY 广告块检测** `markDiscontinuityBlocks`（增强引擎第 5 步）：
  - 广告插入在 m3u8 中常表现为「插入点 `#EXT-X-DISCONTINUITY` → 若干段 → 恢复点 `#EXT-X-DISCONTINUITY`」成对出现；
  - 两个 DISCONTINUITY 之间夹 1-12 段且总时长 ≤ 120s → 整块判为广告（`enhanced_discontinuity_block`）；
  - 单次 DISCONTINUITY（如片头拼接）或夹块过大（正片多段切分）不判，避免误删正片；
- 生效链路：`/api/play`（播放代理，播放即去广告）、`/api/clean/enhanced`（新版增强测试播放）；
- 版本升级 `v0.6.7 → v0.6.8`。

### 验证（真实源 `vip.ffzy-play5.com/20221105/1107_99acb232`）

- 抽帧确认：段 89（5:59 落点）为游戏广告画面（"别再羡慕别人暴富"），段 83/88/94 及前后均为正片动画；DISCONTINUITY 标记在段下标 [0, 89, 94]，广告块=段 89-93（19.6s）；
- `/api/play` 过滤后：段数 320→313（广告块 5 段 + 2 段超短 <1s 段），5:59 处直接接续正片，广告块残留为 0；
- `go vet` / `go build` 通过。

---

## v0.6.7 (2026-09-10) — 全链路增强去广告 + 内嵌/占位广告区间自动跳过

> 所有 m3u8 播放统一走 `/api/play`（服务端内置增强去广告：平台广告域 + 片头片尾边界短簇 + 中插短簇），不再依赖单独请求 `/api/clean`；「压缩在视频里的广告/占位」通过**非正片区间标注**按视频标识（剧名 第N集）绑定区间，播放器自动 seek 跳过；分片地址改为基于请求 Host 的**绝对代理地址**，外部播放器（TVBox 等）也能拿到无广告且不卡顿的播放流。

### 更新内容（[main.go](file:///workspace/main.go)）

- **🔗 全链路统一 `/api/play`（增强引擎同步调用）**：`/api/replace` 的 `ad_skip_url/play_url`、`/api/jx` 的 m3u8 直链全部改为指向 `/api/play?url=<源m3u8>`——服务端内置增强去广告 + 分片代理，播放即无广告且不卡顿，避免 `/api/clean` 二次过滤与源站分片直连；
- **🖥️ 播放代理输出绝对地址**：分片/密钥/子列表地址基于请求 Host 生成 `http(s)://<host>/api/play?url=…`，外部播放器（TVBox/影视 App 等）可直接播放完整去广告流，不再因相对路径 404；
- **⏭️ 内嵌/占位广告区间自动跳过**：非正片区间标注新增**视频标识（Key，空=全局）**维度；`/api/skip` 支持 `?key=` 过滤（返回全局+该视频区间）；播放器（后台 + 外置 `/player`）加载区间后 `timeupdate` 检测并 seek 跳过——**压缩在视频里的广告、占位画面不再看到**；`/api/skip` 列表改为开放接口（播放器读取），增删仍需登录；
- **🧩 官替结果直接带区间**：`/api/replace` 返回 `skip_key`（剧名 第N集）+ `skip_ranges`（该视频专属+全局区间），外置播放页 URL 携带 `key`，内置播放自动加载，无需手工配置；
- **⚡ 播放器不再二次代理**：`proxyPlayURL` 识别本服务地址（`/api/play`、`/api/clean`）直接播放，避免"本服务请求本服务"多一层往返，播放启动更快；
- 版本升级 `v0.6.6 → v0.6.7`。

### 验证

- `go vet` / `go build` 通过；
- 实测：`/api/play` 输出分片为 `http://<host>/api/play?url=…` 绝对地址且中插广告被过滤；`/api/skip?key=` 空 key 仅返回全局区间、指定 key 返回全局+专属区间（含排序）；`/api/skip` 匿名可读、增删需登录；官替/`/api/jx` 输出的直链均指向 `/api/play`。

---

## v0.6.6 (2026-09-10) — 播放代理内置增强去广告 + 修复增强引擎未生效，广告（含中插）去不干净问题

> v0.6.5 播放代理解决了卡顿，但**播放链路改走 `/api/play` 后不再经过原去广告接口**，且「新版增强测试播放」引擎因漏调用 `enhanced(segs)` 实际从未生效 → 插播/广告去不干净。本次：`/api/play` 对 m3u8 列表内置**增强去广告**（平台广告域 + 边界短簇 + 新增中插短簇），任意播放即去广告且不卡顿；同时修复增强引擎未生效的 bug。

### 更新内容（[main.go](file:///workspace/main.go)）

- **🛰️ 播放代理内置去广告**：`/api/play` 对 m3u8 播放列表先经增强引擎过滤再改写分片（master 列表原样改写，其子列表请求同样走代理过滤），播放器拿到的就是无广告列表——**播放即去广告，不再依赖单独请求 `/api/clean`**；
- **📦 播放列表结果缓存**：同一 m3u8 60 秒内复用过滤结果，避免播放器重复拉源站+过滤导致变慢/触发反爬（不缓存分片，分片始终直传）；
- **🐞 修复增强引擎未生效**：`runClean` 漏调用 `enhanced(segs)`，导致「平台广告域关键词 / 片头片尾超短簇」在 `/api/clean/enhanced` 中从未执行。现已补上，增强引擎真正生效；
- **➕ 新增中插短簇检测** `markMidrollClusters`：序列中间连续短段（<6s，1-3 段）且两侧紧邻片段平均时长 ≥ 2.2 倍时判为广告（中插广告/贴片典型形态：短段被长正片包围），补上此前只查片头片尾、漏中插的盲区；
- **🔧 `/api/clean/enhanced` 支持直接输出 m3u8**：默认返回过滤后播放列表（可喂播放器），`format=json` 返回结构化结果；后台「新版增强测试播放 ▶ 直接播放」改走 `/api/play`（服务端已内置增强去广告）；
- 版本升级 `v0.6.5 → v0.6.6`。
...
- `go vet` / `go build` 通过；
- 实测（本地测试源）：
  - 中插广告（4s 短段夹在 10s 正片间）→ 被识别过滤，仅保留正片；
  - 片头 1s×3 logo 簇 + `/ad/` 目录段 → 增强引擎与 `/api/play` 均过滤；
  - master 多码率列表 → 原样改写子列表地址，子列表经 `/api/play` 再过滤；
  - 同一 m3u8 二次请求命中缓存（不再重复拉源）。
