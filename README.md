...
## 当前版本 v5.14.8（2026-09-07）

> M3U8 解析测试播放修复（改用 hls.js 多 CDN 兜底）、片段列表新增地址列、侧边栏支持折叠隐藏让图片/内容显示完整。

### 🎬 M3U8 测试播放修复 + 📋 片段列表 + 📐 侧边栏折叠

- **播放修复**：M3U8 解析测试页播放器改用 **hls.js** 播放 m3u8（原生 video 不支持 HLS，Chrome/Firefox 此前无法播放）；hls.js 改为 bootcdn/jsdelivr/unpkg **多 CDN 兜底加载**，网络不可达时自动切换源；停止/切换播放时正确销毁 hls 实例避免串流。
- **片段列表**：新增「地址」列（绝对地址、超出省略+悬浮显示完整），固定列宽、表格横向滚动不挤压、容器加边框圆角。
- **侧边栏折叠**：顶部栏新增 「◧」折叠按钮，折叠后侧边栏收缩为 64px 窄条（只显示图标），让图片/主内容区显示完整；状态 localStorage 持久化，刷新保持；移动端自动隐藏折叠按钮。
