...
CREATE TABLE IF NOT EXISTS `m3u8_analysis_cache` (
    ...
    `kept_segments` INT DEFAULT 0 COMMENT '保留片段数',
    `original_duration` DECIMAL(10,3) DEFAULT 0 COMMENT '原始时长(秒)',
    `filtered_duration` DECIMAL(10,3) DEFAULT 0 COMMENT '过滤后时长(秒)',
    `saved_duration` DECIMAL(10,3) DEFAULT 0 COMMENT '节省时长(秒)',
