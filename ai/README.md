...
> 🚀 **开箱即用（v0.1.2）**：`config.json` 已预填可用配置——智谱 GLM-4-Flash（免费）+ API Key + 启用 AI 去广告 + AI 智能官替。**无需重新打包编译**，把本目录整个放到二进制同级的 `./ai/` 下，重启主程序即生效（主程序需 v0.6.28+）。其他运行时文件（`resource_sites.json` 等）同理：仓库里有的直接复制到运行目录即可，均不需要重新编译。
>
