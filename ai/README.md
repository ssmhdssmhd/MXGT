...
> 🚀 **开箱即用（v0.1.1）**：`config.json` 已预填可用配置——智谱 GLM-4-Flash（免费）+ API Key + 启用 AI 去广告 + AI 智能官替。**无需重新打包编译**，把本目录整个放到二进制同级的 `./ai/` 下，重启主程序即生效（主程序需 v0.6.28+）。其他运行时文件（`resource_sites.json` 等）同理：仓库里有的直接复制到运行目录即可，均不需要重新编译。
>
> ⚠️ **Key 安全提醒**：仓库内 `config.json` 已写入智谱 Key 便于直接复制使用，但**本仓库是公开仓库，Key 会随代码公开**。若担心泄露：① 复制到服务器后，把 `api_key` 换成你自己的新 Key（智谱控制台可随时重置/删除旧 Key）；② 或删除 `api_key` 值留空，再到后台粘贴 Key 保存（后台保存后 Key 只在服务器本地）。
